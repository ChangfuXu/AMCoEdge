...
class DeepQNetwork:
    ...
    def __init__(self,
        ...
        self.actions_set = np.zeros([2**self.n_actions-1, self.n_actions])
        a_index = 0
        for i in range(1, 6):
            temp_action = np.asarray([c for c in combinations(range(self.n_actions), i)])
            len_ = len(temp_action)
            for j in range(len_):
                self.actions_set[a_index, temp_action[j]] = 1
                a_index = a_index + 1

        self.q_values_num = 2**self.n_actions-1  # which is equals to the number of action set

        # initialize zero memory np.hstack((s, a, [r], s_))
        self.memory = np.zeros((self.memory_size, self.n_features + self.n_actions + 1 + self.n_features))
    ...
    def build_net(self):
        ...
        def build_layers(s, c_names, n_l1, w_initializer, b_initializer):
            ...
            with tf.variable_scope('Q'):
                w4 = tf.get_variable('w4', [n_l1, self.q_values_num], initializer=w_initializer, collections=c_names)
                b4 = tf.get_variable('b4', [1, self.q_values_num], initializer=b_initializer, collections=c_names)
        ...
        with tf.variable_scope('eval_net'):
            ...
            c_names, n_l1, w_initializer, b_initializer = \
                ...
                tf.random_normal_initializer(0.0, 0.3), tf.constant_initializer(0.1)  # config of layers
    ...
    def learn(self):
        ...
        eval_act_index = batch_memory[:, self.n_features:self.n_features + self.n_actions].astype(int)

        # Get all reward indexes in batch memory, where each reward is a single value
        reward = batch_memory[:, self.n_features + self.n_actions]
