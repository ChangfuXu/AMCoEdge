...
def DRLSO_algorithm(DQN_list_, NUM_EPISODE_):
    action_step = 0
            ...
            for b in range(env.n_BSs):
                for n in range(env.n_tasks):
                    ...
                    if state_bnt[b][n][0] != 0:
                        ...
                        reward[b][n] = env.perform_offloading(t, b, n, int(all_actions[b][n]))
...
if __name__ == "__main__":
    ...
    NUM_EPISODE = 200  # The number of episodes
    ...
    DRLSO_algorithm(DQN_list, NUM_EPISODE)

    print('============ Training finished ==========')

    # Plot the average delay varying episodes
    np.savetxt('results/AveMakeSpan_DRLCoEdge_tasks' + str(NUM_TASK) + '_prob' + str(
        np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f' + str(
    ...
    plt.xlabel('Episode')
    plt.savefig('results/AveMakeSpan_DRLCoEdge_tasks' + str(NUM_TASK) + '_prob' + str(
        np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f' + str(
        ES_capacity[4]) + '_episode' + str(NUM_EPISODE) + '.png')
    plt.close()
    # Plot the average failure rate varying episodes
    np.savetxt('results/AveFailRate_DRLCoEdge_tasks' + str(NUM_TASK) + '_prob' + str(
        np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f' + str(
    ...
    plt.xlabel('Episode')
    plt.savefig('results/AveFailRate_DRLCoEdge_tasks' + str(NUM_TASK) + '_prob' + str(
        np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f' + str(
        ES_capacity[4]) + '_episode' + str(NUM_EPISODE) + '.png')
    plt.close()
