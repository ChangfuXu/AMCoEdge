...
class OffloadEnvironment:
    def __init__(self, num_tasks, num_BSs, num_time, deadline, es_capacity, prob):
        # INPUT DATA
        ...
        # self.tran_rate_BSs = np.array([100, 125, 150, 175, 200])  # Mbits/s
    ...
    def perform_offloading(self, t, b, n, action_):
        ...
        if action_ == b:  # Here, the transmission delay and transmission queuing delay equal to 0
            ...
            # Here, the transmission queue doesn't need to update when the BS b is selected
        else:
            action_size = 2
            wait_delays[b] = np.min([(self.proc_queue_len[t][b] + self.proc_queue_bef[t][b]) / self.ES_capacities[b], self.deadline_delay])
            ...
            # Update the queue data size at the selected ES (except BS b) before the current transmission task
            self.tran_delay_bef[t][b] = self.tran_delay_bef[t][b] + allocation_fractions[action_] * n_bit / self.tran_rate_BSs[action_]
