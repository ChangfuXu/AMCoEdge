...
if __name__ == "__main__":
    ...
    NUMB_K = 3  # The number of selected ESs
    ...
    all_actions = np.zeros([NUM_BSs, NUM_TASK, NUMB_K])
    ...
    for i in range(NUM_BSs):
        DQN_list.append(DeepQNetwork(env.n_BSs, env.n_features, env.n_time, NUMB_K,  # GENERATE ENVIRONMENT
                                     learning_rate=0.001,
    ...
    # loss = loss / NUM_BSs
    # np.savetxt('results/Loss_AdaCoEdge_tasks' + str(NUM_TASK) + '_prob' + str(np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_k' + str(NUMB_K) + '_f' + str(ES_capacity[4]) + '_episode' + str(NUM_EPISODE) + '.csv', loss, delimiter=',', fmt='%.4f')
    # plt.figure(1)
    # plt.plot(np.arange(len(loss)), loss)
    # plt.ylabel('Loss')
    # plt.xlabel('Training step')
    # plt.savefig('results/Loss_AdaCoEdge_tasks' + str(NUM_TASK) + '_prob' + str(np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_k' + str(NUMB_K) + '_f' + str(ES_capacity[4]) + '_episode' + str(NUM_EPISODE) + '.png')
    # plt.close()
    # Plot the average delay varying episodes
    np.savetxt('results/AveMakeSpan_SMCoEdge_k' + str(NUMB_K) + '_tasks' + str(NUM_TASK) + '_prob' + str(
    ...
    plt.xlabel('Episode')
    plt.savefig('results/AveMakeSpan_SMCoEdge_k' + str(NUMB_K) + '_tasks' + str(NUM_TASK) + '_prob' + str(
        np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f10-' + str(
        ES_capacity[4]) + '_episode' + str(NUM_EPISODE) + '.png')
    plt.close()
    # Plot the average failure rate varying episodes
    np.savetxt('results/AveFailRate_SMCoEdge_k' + str(NUMB_K) + '_tasks' + str(NUM_TASK) + '_prob' + str(
        np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f10-' + str(
    ...
    plt.xlabel('Episode')
    plt.savefig('results/AveFailRate_SMCoEdge_k' + str(NUMB_K) + '_tasks' + str(NUM_TASK) + '_prob' + str(
        np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f10-' + str(
        ES_capacity[4]) + '_episode' + str(NUM_EPISODE) + '.png')
    plt.close()
