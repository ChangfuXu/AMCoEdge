...
if __name__ == "__main__":
    ...
    # loss = loss / NUM_BSs
    # np.savetxt('results/Loss_AMCoEdge_tasks' + str(NUM_TASK) + '_prob' + str(np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f' + str(ES_capacity[4]) + '_episode' + str(NUM_EPISODE) + '.csv', loss, delimiter=',', fmt='%.4f')
    # plt.figure(1)
    # plt.plot(np.arange(len(loss)), loss)
    # plt.ylabel('Loss')
    # plt.xlabel('Training step')
    # plt.savefig('results/Loss_AMCoEdge_tasks' + str(NUM_TASK) + '_prob' + str(np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f' + str(ES_capacity[4]) + '_episode' + str(NUM_EPISODE) + '.png')
    # plt.close()
    # Plot the average delay varying episodes
    np.savetxt('results/AveMakeSpan_AMCoEdge_tasks' + str(NUM_TASK) + '_prob' + str(np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f' + str(ES_capacity[4]) + '_episode' + str(NUM_EPISODE) + '.csv', aver_make_spans, delimiter=',', fmt='%.4f')
    plt.figure(2)
    plt.plot(aver_make_spans)
    plt.ylabel('Average make-span')
    plt.xlabel('Episode')
    plt.savefig('results/AveMakeSpan_AMCoEdge_tasks' + str(NUM_TASK) + '_prob' + str(np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f' + str(ES_capacity[4]) + '_episode' + str(NUM_EPISODE) + '.png')
    plt.close()
    # Plot the average failure rate varying episodes
    np.savetxt('results/AveFailRate_AMCoEdge_tasks' + str(NUM_TASK) + '_prob' + str(np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f' + str(ES_capacity[4]) + '_episode' + str(NUM_EPISODE) + '.csv', aver_failure_rates, delimiter=',', fmt='%.4f')
    plt.figure(3)
    plt.plot(aver_failure_rates)
    plt.ylabel('Average failure rate')
    plt.xlabel('Episode')
    plt.savefig('results/AveFailRate_AMCoEdge_tasks' + str(NUM_TASK) + '_prob' + str(np.round(TASK_ARRIVAL_PROB, 1)) + '_deadline' + str(np.round(env.deadline_delay, 1)) + '_f' + str(ES_capacity[4]) + '_episode' + str(NUM_EPISODE) + '.png')
    plt.close()
